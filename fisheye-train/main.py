...
def get_args_parser():
    ...
    # Not test batch size > 1. There may be bugs in ./utils/box_ops due to different image sizes in a single card.
    ...
    # * Backbone
    parser.add_argument('--backbone', default='swin-t', type=str,
                        help="Name of the convolutional backbone to use")
    parser.add_argument('--backbone_from_scratch', default=False, action='store_true')
    parser.add_argument('--finetune_early_layers', default=True, action='store_true')
    ...
    parser.add_argument('--batch_norm_type', default='SyncBatchNorm', type=str, 
    ...
    # dataset parameters
    parser.add_argument('--dataset_file', default='loaf')
    parser.add_argument('--coco_path', default='./data/loaf', type=str)
...
def main(args):
    ...
    if args.distributed:
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[args.gpu], find_unused_parameters=args.find_unused_params, broadcast_buffers=False)
