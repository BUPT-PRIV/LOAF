...
#     def forward_memory(self, srcs, masks, pos_embeds, query_embed=None, theta=None):
#         """
#         Input:
#             - srcs: List([bs, c, h, w])
#             - masks: List([bs, h, w])
...
#             #rotate
#             rot_mat = self.get_rot_mat(theta)[None, ...].repeat(bs,1,1)
#             grid = F.affine_grid(rot_mat, src.size())
#             src = F.grid_sample(src, grid)
#             pos_embed = F.grid_sample(pos_embed, grid)
#             grid = F.affine_grid(rot_mat, mask.unsqueeze(1).size())
#             mask = F.grid_sample(mask.unsqueeze(1).float(), grid).squeeze(1).bool()
...
#     def forward(self, srcs, masks, pos_embeds, query_embed=None, memory_equ=None, spatial_shapes_equ=None, level_start_index_equ=None, valid_ratios_equ=None, mask_flatten_equ=None):
#         """
#         Input:
#             - srcs: List([bs, c, h, w])
#             - masks: List([bs, h, w])
...
#             # equ
#             if memory_equ!=None:
#                 output_memory, output_proposals = self.gen_encoder_output_proposals(memory_equ, mask_flatten_equ, spatial_shapes_equ)
...
#                 reference_points_equ = topk_coords_unact.sigmoid()
#                 init_reference_out = reference_points_equ
#                 pos_trans_out = self.pos_trans_norm(self.pos_trans(self.get_proposal_pos_embed(topk_coords_unact)))
#                 query_embed, tgt_equ = torch.split(pos_trans_out, c, dim=2)
#             else:
...
#         if memory_equ!=None:
#             hs, inter_references = self.decoder(tgt_equ, reference_points_equ, memory,
...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, memory_equ=None, spatial_shapes_equ=None, level_start_index_equ=None, valid_ratios_equ=None, mask_flatten_equ=None):
        ...
        if memory_equ!=None:
            hs, inter_references = self.decoder(tgt, reference_points, memory_equ,
                                            spatial_shapes_equ, level_start_index_equ, valid_ratios_equ, 
                                            query_pos=query_embed if not self.use_dab else None, 
                                            src_padding_mask=mask_flatten_equ)
